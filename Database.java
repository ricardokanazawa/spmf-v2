...
public class Database {
	...
	public void addTransaction(String itemsString[]){
		...
		for(String itemString : itemsString){
			...
			int item = Integer.parseInt(itemString.split("\\|")[1]);
			...
			map.put(Integer.parseInt(itemString.split("\\|")[1]), itemString.split("\\|")[0]);
		}
	...
	public boolean isProcedure(int code) {
		String codigoTabela = map.get(code); 
		
		if (codigoTabela != null)
		{
			if (Integer.parseInt(codigoTabela) == 22)
				return true;	
		}
		else {
			return false;
